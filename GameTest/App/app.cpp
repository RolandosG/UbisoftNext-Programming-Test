...
{	
	...
	void PrintTwo(const float x, const float y, const char* text, float value,
		const float r, const float g, const float b, void* font) {

		// Use a stringstream to format the string
		std::stringstream ss;
		ss << text << value;
		std::string formattedString = ss.str();

		// Call existing Print function with the formatted string
		Print(x, y, formattedString.c_str(), r, g, b, font);
	}
